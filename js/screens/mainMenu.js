...
/*		scene.append(button = new Button({
			position: Ex(position.x - 60, position.y - 100, 50, 50),
			size: Ex(120, 120),
			radius: 60,
			fill: "#aaa7",
	...
	function decideMode(mode, force = false) {
		...
		startAnimation(x => {
			...
			let sharedY = -94 + clamp01(ease.quint.out(x / 1000)) * 200;
			scene.$profile.position.y = sharedY;
			scene.$switchbtn.position.y = sharedY;
			scene.$menubtn.position.y = sharedY;
			console.log(sharedY + " the j")
	...
	console.log(scene.$profile.size);
	scene.$profile.append(new Gembar({
		position: Ex(98, 2),
		size: Ex(-100, -4, 100, 100), 
		progress: Number(game.stats.exp) / Number(goal),
		fill: "#000a",
	}), "progress");
	scene.$profile.append(new Rect({
		position: Ex(2, 2),
		size: Ex(94, -4, 0, 100),
		fill: "#000a",
	}), "rankbox");
	scene.$profile.$rankbox.append(new Label({
		position: Ex(0, 0, 50, 50), // somehow, this should be offset elsewhere...?
		text: game.stats.level.toLocaleString("en-US"),
		scale: 25,
	}), "rank");

	scene.append(new ButtonWithText("⇆", () => {
		popups.switcher();
	}, {
	...
	scene.append(Object.assign(new Base(),{
		onupdate: () => {
			//console.log("the j")
	...
	startAnimation(x => {
		for (let a in modeButtons) {
			//console.log(modeButtons[a]);
			modeButtons[a].position.ey = 150 - clamp01(ease.cubic.out((x - 20 * a) / 1000)) * 100;
		};
		let sharedY = 104 - clamp01(ease.cubic.out((x - 250) / 1000)) * 200;
		scene.$profile.position.y = sharedY;
		scene.$switchbtn.position.y = sharedY;
		scene.$menubtn.position.y = sharedY;
